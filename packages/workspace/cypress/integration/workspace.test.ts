...
import { expectError } from '../support/utils';
...
describe('Workspace tests', () => {
  ...
  it('Call createWorkspace with a token with no configuration available is rejected with error', () => {
    ...
    return expectError(workspaceFactory.createWorkspace({ token: 'http://127.0.0.1:9999' }), repositoryNotFoundError);
  });

  it('Call createWorkspace with a token with invalid configuration is rejected with error', () => {
    const workspaceFactory = new WorkspaceFactory();
    return expectError(
      workspaceFactory.createWorkspace({ token: 'http://127.0.0.1:3001/configurations/invalid' }),
      configWrongFormatError
    );
  });
  ...
  invalidCreateWorkspaceRequest.forEach((invalidToken: any) => {
    it(`Call createWorkspace with a token with invalid format is rejected with error (${invalidToken})`, () => {
      const workspaceFactory = new WorkspaceFactory();
      return expectError(workspaceFactory.createWorkspace({ token: invalidToken }), createWorkspaceWrongRequestError);
    });
  });

  const invalidConfigurationTypes = [' ', {}, { test: 'test' }, [], ['test'], null, true, false, 0, -1];
  invalidConfigurationTypes.forEach((invalidConfigProvider) => {
    it(`Call createWorkspace with an invalid configProvider is rejected with error (${invalidConfigProvider})`, () => {
      const workspaceFactory = new WorkspaceFactory();
      ...
      return expectError(
        workspaceFactory.createWorkspace({
          token: 'http://127.0.0.1:3001/configurations/valid',
          // @ts-ignore
          configProvider: invalidConfigProvider,
        }),
        configNotLoadedError(new Error(errorMessageFromConfigurationService))
      );
    });
  });
